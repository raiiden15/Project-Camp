...
const verify_email = async_handler(async (req, res) => {
    ...
    let hashed_token = crypto.createHash("sha256").update(verification_token).digest("hex")
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, {is_email_verified: true} , "Email is verified")
        )
});
...
const refresh_access_token = async_handler(async (req, res) => {
    ...
    try {
        const decoded_refresh_token = jwt.verify(incoming_refresh_token, process.env.REFRESH_TOKEN_SECRET);
        ...
        const {access_token, refresh_token: new_refresh_token} = await generate_tokens(user._id)

        user.refresh_token = new_refresh_token
        await user.save();

        return res.status(200).cookie("accessToken", access_token, options).cookie("refreshToken", new_refresh_token, options).json(
            new ApiResponse(
                200, 
                {
...
export { register_user, login, logout, get_current_user, verify_email, resent_email_verification, refresh_access_token };
